...
async def init_db():
    ...
    for k in UNITS:
        if k not in existing:
            await db.execute(f'ALTER TABLE users ADD COLUMN {k} INTEGER NOT NULL DEFAULT 0')
    ...
    # Database-level safety rails: even an accidental negative amount in an admin command
    # cannot create negative balances, armies, taxes, or invalid farm levels.
    await db.execute('''CREATE TRIGGER IF NOT EXISTS users_guard_values BEFORE UPDATE ON users
        WHEN NEW.balance < 0 OR NEW.tax < 0 OR NEW.tax > 1000000 OR NEW.farm_level < 1 OR NEW.farm_level > 10
        OR NEW.attacks_won < 0 OR NEW.attacks_lost < 0
        OR NEW.soldier < 0 OR NEW.interceptor < 0 OR NEW.drone < 0 OR NEW.bmp < 0
        OR NEW.tank < 0 OR NEW.helicopter < 0 OR NEW.plane < 0 OR NEW.missile < 0 OR NEW.artillery < 0
        BEGIN SELECT RAISE(ABORT, 'invalid user state'); END''')
    await db.execute('''CREATE TRIGGER IF NOT EXISTS users_guard_insert BEFORE INSERT ON users
        WHEN NEW.balance < 0 OR NEW.tax < 0 OR NEW.tax > 1000000 OR NEW.farm_level < 1 OR NEW.farm_level > 10
        OR NEW.attacks_won < 0 OR NEW.attacks_lost < 0
        OR NEW.soldier < 0 OR NEW.interceptor < 0 OR NEW.drone < 0 OR NEW.bmp < 0
        OR NEW.tank < 0 OR NEW.helicopter < 0 OR NEW.plane < 0 OR NEW.missile < 0 OR NEW.artillery < 0
...
async def top_users(limit=10):
    db = await connect()
    cur = await db.execute('SELECT user_id,username,balance,farm_level FROM users ORDER BY balance DESC LIMIT ?', (max(1, min(100, int(limit))),))
