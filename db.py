...
async def connect():
    db=await aiosqlite.connect(DB_PATH); db.row_factory=aiosqlite.Row; return db
...
async def ensure_user(uid,username=''):
    ...
    await db.execute('INSERT OR IGNORE INTO users(user_id,username,last_payout) VALUES(?,?,?)',(uid,username or '',stamp)); await db.execute('UPDATE users SET username=? WHERE user_id=?',(username or '',uid))
    await db.execute('INSERT OR IGNORE INTO admins(user_id) SELECT ? WHERE ? IN (SELECT value FROM settings WHERE key="owner_id")',(uid,str(uid)))
    await db.commit(); await db.close()

async def user(uid):
    db=await connect(); cur=await db.execute('SELECT * FROM users WHERE user_id=?',(uid,)); r=await cur.fetchone(); await db.close(); return r
async def setting(key,default=None):
    db=await connect(); cur=await db.execute('SELECT value FROM settings WHERE key=?',(key,)); r=await cur.fetchone(); await db.close(); return r['value'] if r else default
...
async def is_admin(uid,owner_id):
    ...
    db=await connect(); cur=await db.execute('SELECT 1 FROM admins WHERE user_id=?',(uid,)); r=await cur.fetchone(); await db.close(); return bool(r)
async def top_users(limit=10):
    db=await connect(); cur=await db.execute('SELECT user_id,username,balance,farm_level FROM users ORDER BY balance DESC LIMIT ?',(limit,)); r=await cur.fetchall(); await db.close(); return r
async def all_user_ids():
    db=await connect(); cur=await db.execute('SELECT user_id FROM users'); r=await cur.fetchall(); await db.close(); return [x[0] for x in r]
async def users_count():
    db=await connect(); cur=await db.execute('SELECT COUNT(*) c FROM users'); r=await cur.fetchone(); await db.close(); return r['c']
